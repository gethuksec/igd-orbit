...
- **`ws-request-stock` merged to main (2026-09-12, user-approved):** merge commit `b4d727b` — main now carries everything the preview runs. Merged tree verified: content-identical to `ws-deploy-po-s1`, **373/373** BE tests, clean BE+FE builds. Prod rebuild from `main` (retiring the deploy branch) = separate deploy-gate item.
- **Merged 2026-09-12:** `origin/main` now **`3e7a35e`** (FF from `d008a2b`) — contains **S1–S5** (PO v2, overdue, edit-after-approval, retur + completion lifecycle, barcode labels). FF executed via fresh worktree + `push origin HEAD:main`; tree equality verified. `ws-request-stock` merged separately the same day (`b4d727b`) — main = full preview content; prod rebuild from main = separate deploy-gate item.
...
- **Completion lifecycle (2026-09-12, user request):** returns stay `open` until the goods are physically shipped back; **Tandai Selesai** on the detail → PO-style confirm modal (checkbox gate, like create-PO) → central-bad −qty + audit movement; `completed` + completedAt/By stored. List shows status badges; stat card counts open returns.
...
- E2E (preview 2026-09-12): manual return RTP-20260912-810979 (good 50→49 / bad 0→1) + PO return RTP-20260912-115031 (good 49→47 / bad 1→3); duplicate guard blocks a 2nd return per invoice; DB movements verified; **completion E2E** — RTP-20260912-810979 marked selesai via modal → bad 3→2 + OUT movement + status/actor verified.
...
- FE placement (user call 2026-09-12 — **no dedicated menu**): labels live in **Goods Receipt** — GR list has a **Label Pending (N)** button (bulk print across GRs); GR detail (approved) has a **Label Barcode** card (per-GR jobs, Cetak button, printed-by). Settings moved to **Administrator → General → Barcode Setting** (submenu; form + live label preview). Print sheet `/purchasing/label-print/sheet?ids=` renders labels at exact mm — CODE128 via JsBarcode / QR via qrcode, thermal (1 label/page) or A4 grid, auto `window.print()`, then "Tandai Sudah Dicetak" (actor + time recorded) → returns to the previous page.
- Auto-print decision: **ship as-is (one extra click: approve → Cetak)**; real auto-print needs a local print agent — revisit when the client's label format + printer model arrive.
