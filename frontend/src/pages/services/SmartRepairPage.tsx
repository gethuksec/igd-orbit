...
function MoneyInput({ value, onChange, className, disabled }: { value: number; onChange: (n: number) => void; className?: string; disabled?: boolean }) {
  return <Input inputMode="numeric" value={value ? value.toLocaleString('id-ID') : ''} onChange={(e) => onChange(Number(e.target.value.replace(/\D/g, '') || 0))} className={className} disabled={disabled} />;
}
...
import LockModal, { LOCK_TYPE_LABELS, type LockType } from '@/components/services/LockModal';
...
export default function SmartRepairPage() {
  ...
  // IGDERP-187: price override gated — tier auto-fill is the default
  const { hasPermission } = usePermissions();
  const canEditPrice = hasPermission('service.price.edit');
  ...
  // IGDERP-187: customer tier drives layanan price auto-fill (satu device = satu nota)
  const [customerTierId, setCustomerTierId] = useState<string | null>(null);
  const [layananNotes, setLayananNotes] = useState<Record<string, string[]>>({});
  const [newLayananCost, setNewLayananCost] = useState('');
  // IGDERP-185: lock credential lives in a modal (entry row on the form)
  ...
  const { data: deviceTypesMaster = [] } = useQuery({ queryKey: ['smart-repair', 'device-types'], queryFn: () => fetchList('/api/v1/device-types/active') });
  ...
  // IGDERP-187: effective tier (registered customer only — walk-in uses base price)
  const effectiveTierId = form.customerId ? customerTierId : null;
  const tierPriceOf = (st: any) => {
    const raw = effectiveTierId ? (st?.tierPricing as any)?.[effectiveTierId] : undefined;
    const n = Number(raw);
    return raw != null && Number.isFinite(n) && n > 0 ? n : Number(st?.basePrice || 0);
  };
  const { data: customerTiers = [] } = useQuery({
    queryKey: ['smart-repair', 'customer-tiers'],
    queryFn: () => fetchList('/api/v1/customers/tiers'),
  });
  const tierName = useMemo(() => (effectiveTierId ? (customerTiers as any[]).find((t: any) => t.id === effectiveTierId)?.name || null : null), [effectiveTierId, customerTiers]);
  ...
  const handleSave = () => {
    ...
    if (lockType !== 'none' && !lockValue) return toast.error('Kunci layar belum diisi');
    ...
    saveMutation.mutate({
      ...
      deviceLockType: lockType, devicePassword: lockValue || undefined,
  ...
  const chooseCustomer = (customer: any) => {
    ...
    setCustomerTierId(customer.tierId || null); // IGDERP-187: tier-keyed price auto-fill
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-gray-50">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
        ...
        <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Ringkasan Biaya</CardTitle><p className="text-xs text-muted-foreground">Auto dari layanan &amp; barang — Estimasi tetap bisa diubah</p></CardHeader><CardContent><div><Label><span className="text-red-500">*</span> Teknisi</Label><Select value={form.technicianId} onValueChange={(technicianId) => setF({ technicianId })} className="mt-1 max-w-md"><option value="">{form.outlet ? 'Pilih Teknisi' : 'Pilih Outlet terlebih dahulu'}</option>{technicians.map((t: any) => <option key={t.id} value={t.id}>{t.fullName || t.email}</option>)}</Select></div><Separator className="my-4" /><div className="grid grid-cols-1 gap-4 lg:grid-cols-2"><div className="space-y-2 text-sm">{layananPicked.map((l: any) => <div key={l.id} className="flex justify-between border-b border-dashed pb-2"><span>Layanan · {l.name}</span><span>{formatCurrency(Number(layananCost[l.id] || 0))}</span></div>)}<div className="flex justify-between font-semibold"><span>Total Layanan</span><span>{formatCurrency(layananTotal)}</span></div>{partsRows.map((r) => <div key={r.id} className="flex justify-between border-b border-dashed pb-2"><span>Barang · {r.productName} ×{r.quantity}</span><span>{formatCurrency(r.total)}</span></div>)}</div><div className="space-y-3"><div><Label>Estimasi Biaya <span className="text-muted-foreground">(bisa diubah, min. total)</span></Label><MoneyInput value={estimasiManual ? Number(estimasiValue || 0) : estimatedAuto} onChange={(n) => { setEstimasiManual(true); setEstimasiValue(n ? String(n) : ''); }} className="mt-1 bg-red-50 text-right" disabled={!canEditPrice} /><p className="mt-1 text-xs text-muted-foreground">Min. total layanan + barang: {formatCurrency(estimatedAuto)}{!canEditPrice && ' · Harga mengikuti tier (butuh izin Ubah Harga untuk edit)'}</p></div><div><Label>Uang Muka</Label><MoneyInput value={Number(form.downPayment || 0)} onChange={(n) => setF({ downPayment: n ? String(n) : '' })} className="mt-1 text-right" /></div><div className="flex justify-between border-t pt-3 text-base font-bold text-red-600"><span>Sisa (Estimasi − UM)</span><span>{formatCurrency(remaining)}</span></div></div></div></CardContent></Card>
