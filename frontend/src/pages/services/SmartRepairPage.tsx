...
export default function SmartRepairPage() {
  ...
  const [newLayananTag, setNewLayananTag] = useState('');
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate({
      ...
      layananItems: selectedLayanan.map((id) => ({ serviceTypeId: id, estimatedCost: Number(layananCost[id] || 0), notes: layananNotes[id] || undefined })),
  ...
  const addLayanan = () => {
    ...
    if (newLayananTag.trim()) setLayananNotes((notes) => ({ ...notes, [newLayanan]: newLayananTag.trim() }));
    setNewLayanan(''); setNewLayananCost(''); setNewLayananTag(''); setOpenLayanan(false);
  };
