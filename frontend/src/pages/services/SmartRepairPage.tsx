...
export default function SmartRepairPage() {
  ...
  const [form, setForm] = useState({
    ...
    isUnderWarranty: false, warrantyDays: '30',
  });
  ...
  const { data: productResults = [] } = useQuery({
    ...
    queryFn: () => fetchList('/api/v1/pos/products?q=' + encodeURIComponent(productSearch) + '&limit=10' + (form.warehouseId ? '&warehouseId=' + encodeURIComponent(form.warehouseId) : '')),
  });
  ...
  const { data: customersRes } = useQuery({ queryKey: ['smart-repair', 'customers-all'], queryFn: () => customersService.getAll({ limit: 100 }) });
  ...
  const subdistricts = useMemo(() => Array.from(new Set((((customersRes as any)?.data || []).map((c: any) => c.subdistrict).filter(Boolean)))) as string[], [customersRes]);
  ...
  // Estimasi Selesai: Tgl Terima + max SLA, unless CS edited it manually
  useEffect(() => {
    ...
    const maxSla = Math.max(...picked.map((l) => Number(l.slaHours || 0)));
    setForm((f) => ({ ...f, promisedDate: isoLocal(new Date(new Date(f.receivedDate).getTime() + maxSla * 3600 * 1000)) }));
  ...
  const handleSave = () => {
    ...
    if (partsRows.some((r) => r.productId && r.warehouseId !== form.warehouseId)) return toast.error('Gudang berubah — hapus & pilih ulang barang');
    saveMutation.mutate({
      ...
      estimatedCost: estimatedTotal, finalPrice: estimatedTotal, downPayment: downPayment || undefined, warrantyDays: form.isUnderWarranty ? Math.max(0, Number(form.warrantyDays || 30)) : undefined,
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-gray-50">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
        ...
        <Card><CardContent className="p-4">
          <div className="mb-3 flex items-end gap-2"><div className="min-w-0 flex-1"><Label><span className="text-red-500">*</span> Pelanggan</Label><div className="relative mt-1"><Input value={customerSearch || form.customerName} onChange={(e) => { setCustomerSearch(e.target.value); setShowCustomerResults(true); setF({ customerName: e.target.value }); }} placeholder="Cari pelanggan..." /><Search className="absolute right-3 top-2.5 h-4 w-4 text-muted-foreground" /></div>{showCustomerResults && customerResults.length > 0 && <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-40 overflow-auto rounded-md border bg-white shadow">{customerResults.map((c: any) => <button key={c.id} type="button" className="block w-full border-b px-3 py-2 text-left text-sm hover:bg-muted" onClick={() => chooseCustomer(c)}>{c.name} · {c.phone}</button>)}</div>}</div><Button variant="outline" size="sm" className="shrink-0 border-primary text-primary" onClick={() => setOpenCustomer(true)}><Plus className="h-4 w-4" /> Tambah</Button></div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><Label><span className="text-red-500">*</span> No. HP</Label><Input value={form.customerPhone} onChange={(e) => setF({ customerPhone: e.target.value })} className="mt-1" /><p className="mt-1 text-xs text-muted-foreground">Format: 08… atau +62… — dicek saat simpan.</p></div><div><Label><span className="text-red-500">*</span> Kecamatan</Label><Input value={form.customerSubdistrict} onChange={(e) => setF({ customerSubdistrict: e.target.value })} list="sr-kecamatan" className="mt-1" /><datalist id="sr-kecamatan">{subdistricts.map((s) => <option key={s} value={s} />)}</datalist></div></div>
        </CardContent></Card>

        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <Card><CardContent className="p-4"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><Label><span className="text-red-500">*</span> Jenis Perangkat</Label><Select value={form.deviceType} onValueChange={(deviceType) => setF({ deviceType: deviceType as typeof form.deviceType })} className="mt-1"><option value="handphone">Handphone</option><option value="laptop">Laptop</option><option value="tablet">Tablet</option><option value="other">Lainnya</option></Select></div><div><Label>Nama Barang / Tipe</Label><Input value={form.deviceUnit} onChange={(e) => setF({ deviceUnit: e.target.value })} placeholder="Samsung A54 / iPhone 13..." list="sr-brands" className="mt-1" /><datalist id="sr-brands">{brandNames.map((b) => <option key={b} value={b} />)}</datalist></div><div><Label>Serial Number</Label><Input value={form.deviceSerial} onChange={(e) => setF({ deviceSerial: e.target.value })} className="mt-1" /></div><div><Label>Warna</Label><Select value={form.deviceColor} onValueChange={(deviceColor) => setF({ deviceColor })} className="mt-1"><option value="">Pilih Warna</option>{colorNames.map((c) => <option key={c} value={c}>{c}</option>)}</Select></div></div><div className="mt-3"><Label><span className="text-red-500">*</span> Deskripsi &amp; Kondisi</Label><Textarea value={form.complaint} onChange={(e) => setF({ complaint: e.target.value })} className="mt-1 min-h-20" /></div><div className="mt-3 flex items-center gap-2"><label className="flex cursor-pointer items-center gap-2 text-sm"><Checkbox checked={form.isUnderWarranty} onCheckedChange={(checked) => setF({ isUnderWarranty: Boolean(checked) })} /> Dalam Garansi (servis)</label>{form.isUnderWarranty && <div className="flex items-center gap-1"><Input type="number" min={0} value={form.warrantyDays} onChange={(e) => setF({ warrantyDays: e.target.value })} className="h-8 w-20" /><span className="text-xs text-muted-foreground">hari</span></div>}</div></CardContent></Card>
