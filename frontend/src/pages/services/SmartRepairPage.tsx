...
function TagCombobox({ value, onChange, suggestions, placeholder }: { value: string[]; onChange: (tags: string[]) => void; suggestions: string[]; placeholder?: string }) {
  const [open, setOpen] = useState(false);
  const [input, setInput] = useState('');
  const q = input.trim();
  const filtered = suggestions.filter((s) => s.toLowerCase().includes(q.toLowerCase()) && !value.includes(s));
  const canAdd = q.length > 0 && !value.some((v) => v.toLowerCase() === q.toLowerCase());
  const commit = (tag: string) => { const t = tag.trim(); if (!t) return; if (!value.some((v) => v.toLowerCase() === t.toLowerCase())) onChange([...value, t]); setInput(''); setOpen(false); };
  return (
    <div className="relative">
      <div className="flex min-h-7 flex-wrap items-center gap-1 rounded-md border border-input bg-background px-2 py-1" onClick={() => setOpen(true)}>
        ...
        <input value={input} onChange={(e) => { setInput(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 120)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (filtered.length) commit(filtered[0]); else if (canAdd) commit(q); } if (e.key === 'Escape') setOpen(false); }} placeholder={value.length ? '' : (placeholder || 'Pilih / ketik tag...')} className="min-w-20 flex-1 bg-transparent text-xs outline-none" />
      </div>
      {open && (filtered.length > 0 || canAdd) && <div className="absolute left-0 right-0 top-full z-30 mt-1 max-h-40 overflow-auto rounded-md border bg-white shadow">{filtered.map((s) => <button key={s} type="button" onMouseDown={(e) => { e.preventDefault(); commit(s); }} className="block w-full px-3 py-2 text-left text-xs hover:bg-muted">{s}</button>)}{canAdd && <button type="button" onMouseDown={(e) => { e.preventDefault(); commit(q); }} className="block w-full border-t px-3 py-2 text-left text-xs font-medium text-primary hover:bg-muted">+ Tambah "{q}"</button>}</div>}
    </div>
  );
}
