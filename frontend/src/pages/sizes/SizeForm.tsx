...
import { Save, X, Loader2, ArrowLeft, Maximize } from "lucide-react";
...
export default function SizeForm() {
  ...
  const { data: size, isLoading: loadingSize } = useQuery({
    queryKey: ["size", id],
    queryFn: async () => {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => {
      ...
      if (isEdit) {
        return api.put(`/sizes/${id}`, submitData);
      }
      return api.post("/sizes", submitData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sizes"] });
      toast.success(isEdit ? "Ukuran berhasil diupdate" : "Ukuran berhasil ditambahkan");
      navigate("/sizes");
    },
    onError: (error: any) => {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-primary-100">
                {isEdit ? "Ubah informasi ukuran" : "Tambahkan ukuran baru"}
              </p>
            </div>
          </div>
          <button
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div className="p-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900">Informasi Ukuran</h2>
          </div>
            ...
            {isEdit && size?.code && (
              <div>
                ...
                <input
                  ...
                  placeholder="Kode ukuran (auto-generated)"
                />
                <p className="text-xs text-gray-500 mt-1">Kode ukuran dibuat otomatis dan tidak dapat diubah</p>
              </div>
            )}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
