    ...
    <div className="w-full space-y-3">
      ...
      <div className="bg-gradient-to-r from-primary-600 to-primary-500 rounded-xl shadow-lg p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold mb-2">Manajemen Brand</h1>
            <p className="text-primary-100 text-lg">Kelola brand produk</p>
          </div>
          <Link to="/brands/new">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 hover:shadow-lg transition-all duration-300 group">
          ...
          <p className="text-sm font-medium text-gray-600 mb-1">Total Brand</p>
          <h3 className="text-3xl font-bold text-gray-900 mb-1">{isLoading ? '-' : pagination.total}</h3>
          <p className="text-xs text-gray-500">Semua brand terdaftar</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 hover:shadow-lg transition-all duration-300 group">
          ...
          <h3 className="text-3xl font-bold text-gray-900 mb-1">
            {isLoading ? '-' : brands.reduce((acc: number, b: any) => acc + (b.productCount || 0), 0)}
          </h3>
          <p className="text-xs text-gray-500">Dari semua brand</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 hover:shadow-lg transition-all duration-300 group">
          ...
          <p className="text-sm font-medium text-gray-600 mb-1">Brand Aktif</p>
          <h3 className="text-3xl font-bold text-gray-900 mb-1">
            {isLoading ? '-' : brands.filter((b: any) => b.isActive).length}
          </h3>
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-gray-600 font-semibold text-lg">Memuat data brand...</p>
                    </div>
                  </td>
                </tr>
              ) : brands.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-gray-600 font-semibold text-lg">Tidak ada brand ditemukan</p>
                      <Link to="/brands/new">
                ...
                brands.map((brand: any) => (
                  ...
                  >
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-1 font-mono">{brand.code || '-'}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="text-sm font-semibold text-gray-900">{brand.productCount || 0}</div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span
          ...
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-3 border-t border-gray-200">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">
                Menampilkan <span className="font-bold text-gray-900">{brands.length}</span> dari{' '}
                <span className="font-bold text-gray-900">{pagination.total}</span> brand
